...
export default function AdminDashboard() {
  ...
  const suspendUser = (id) => {
    axios.put(`https://community-website-aitsp-1.onrender.com/admin/suspend/${id}`)
      .then(() => window.location.reload());
  };

  const deleteUser = (id) => {
    axios.delete(`https://community-website-aitsp-1.onrender.com/admin/user/${id}`)
      .then(() => window.location.reload());
  };

  const approvePost = (id) => {
    axios.put(`https://community-website-aitsp-1.onrender.com/admin/approve/${id}`)
      .then(() => window.location.reload());
  };

  const deletePost = (id) => {
    axios.delete(`https://community-website-aitsp-1.onrender.com/admin/post/${id}`)
      .then(() => window.location.reload());
  };
